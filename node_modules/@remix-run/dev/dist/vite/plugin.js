  ...
  let getRemixManifestForDev = async () => {
    ...
    for (let [key, route] of Object.entries(ctx.remixConfig.routes)) {
      ...
      routes[key] = {
        ...
        index: route.index,
        caseSensitive: route.caseSensitive,
        module: path__namespace.posix.join(ctx.remixConfig.publicPath, `${resolveFileUrl.resolveFileUrl(ctx, resolveRelativeRouteFilePath(route, ctx.remixConfig))}`),
        hasAction: sourceExports.includes("action"),
        hasLoader: sourceExports.includes("loader"),
        hasClientAction: sourceExports.includes("clientAction"),
        hasClientLoader: sourceExports.includes("clientLoader"),
    ...
    return {
      ...
      url: path__namespace.posix.join(ctx.remixConfig.publicPath, vmod.url(browserManifestId)),
      hmr: {
        runtime: path__namespace.posix.join(ctx.remixConfig.publicPath, vmod.url(injectHmrRuntimeId))
      },
      entry: {
        module: path__namespace.posix.join(ctx.remixConfig.publicPath, resolveFileUrl.resolveFileUrl(ctx, ctx.entryClientFilePath)),
        imports: []
      },
      routes
    };
...
async function getRouteMetadata(ctx, viteChildCompiler, route, readRouteFile) {
  ...
  let info = {
    ...
    index: route.index,
    caseSensitive: route.caseSensitive,
    url: path__namespace.posix.join(ctx.remixConfig.publicPath, "/" + path__namespace.relative(ctx.rootDirectory, resolveRelativeRouteFilePath(route, ctx.remixConfig))),
    module: path__namespace.posix.join(ctx.remixConfig.publicPath, `${resolveFileUrl.resolveFileUrl(ctx, resolveRelativeRouteFilePath(route, ctx.remixConfig))}?import`),
